...
payload_test = {
    "workers": [
        {"id": "w101", "types": ["A"]},
        {"id": "w102", "types": ["C"]},
        {"id": "w103", "types": ["E", "H"], "busy_windows": [(20, 60)]},
        {"id": "w104", "types": ["G"]},
        {"id": "w105", "types": ["B"]},
        {"id": "w106", "types": ["D", "F"]},
    ],
    ...
    "tasks": [
        ...
        {"id": "t6", "type": "F", "duration": 3, "earliest_start": 0, "deadline": 10},
...
def main():
    ...
    for w in payload["workers"]:
        ...
        new_busy_windows = []
        if "busy_windows" in w:
            for s, t in w["busy_windows"]:
                s = int(s * 1e6)
                t = int(t * 1e6)
                new_busy_windows.append((s, t))
            w["busy_windows"] = new_busy_windows
        print(f"Worker {w['id']} types:  {', '.join(w['types'])}")

    for m in payload["machines"]:
        m["types"] = sorted(m["types"])
        new_busy_windows = []
        if "busy_windows" in m:
            for s, t in m["busy_windows"]:
                s = int(s * 1e6)
                t = int(t * 1e6)
                new_busy_windows.append((s, t))
            m["busy_windows"] = new_busy_windows
        print(f"Machine {m['id']} types: {', '.join(m['types'])}")

    for t in payload["tasks"]:
        t["duration"] = int(t["duration"] * 1e6)
        t["earliest_start"] = int(t["earliest_start"] * 1e6)
        t["deadline"] = int(t["deadline"] * 1e6)
        print(
